...
parser.add_argument('--data_dir', default='/model/dataSplits', help="Directory containing the dataset")
...
def train(model, optimizer, loss_fn, dataloader, metrics, params):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (train_batch, labels_batch) in enumerate(dataloader):
            ...
            if params.cuda:
                train_batch, labels_batch = train_batch.cuda(async=True).float(), labels_batch.cuda(async=True)
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, loss_fn, metrics, params, model_dir,
    ...
    for epoch in range(params.num_epochs):
        ...
        val_acc = val_metrics['patient-acc']
...
if __name__ == '__main__':
    ...
    if args.model_id.startswith('resnet'):
        ...
        model.classifier = nn.Sequential(
            #nn.Linear(num_ftrs, params.last_layer) 
	    #nn.Dropout(p=params.dropout, inplace=False)
	    nn.Linear(num_ftrs, 1),
            nn.Sigmoid())
        
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=params.learning_rate, weight_decay=params.L2_decay)
